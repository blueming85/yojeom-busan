...
directories_to_create = [
    ...
    PLANS_DIR, PLANS_PDF_DIR, PLANS_MD_DIR,  # 업무계획
    RESTAURANT_DIR, RESTAURANT_MD_DIR,  # 맛집
    POLICY_DIR, POLICY_MD_DIR  # 정책지도
]
...
PLAN_DEPARTMENTS = [
    ("🏠 전체", "전체"),
    ("🏛️ 기획감사", ["기획관", "기획조정실", "대변인실", "감사위원회", "부산광역시"]),
...
# 정책지도는 맛집과 동일한 4권역 사용
POLICY_REGIONS = RESTAURANT_REGIONS
AVAILABLE_POLICY_REGIONS = AVAILABLE_RESTAURANT_REGIONS
POLICY_REGION_COLORS = RESTAURANT_REGION_COLORS
...
def get_env_info() -> dict:
    ...
    return {
        ...
        "restaurant_dir": str(RESTAURANT_DIR),
        "policy_dir": str(POLICY_DIR),  # 정책지도 디렉토리 추가
        "api_available": OPENAI_API_KEY is not None,
        "kakao_api_available": KAKAO_REST_API_KEY is not None,
...
def validate_config() -> dict:
    ...
    # 디렉토리 체크
    required_dirs = [DATA_DIR, PDF_DIR, MD_DIR, PLANS_DIR, PLANS_PDF_DIR, PLANS_MD_DIR, 
                    RESTAURANT_DIR, RESTAURANT_MD_DIR, POLICY_DIR, POLICY_MD_DIR]
...
if __name__ == "__main__":
    ...
    print(f"업무계획 디렉토리: {env_info['plans_dir']}")
    print(f"맛집 디렉토리: {env_info['restaurant_dir']}")
    print(f"정책지도 디렉토리: {env_info['policy_dir']}")  # 정책지도 디렉토리 추가
    print(f"API 사용 가능: {env_info['api_available']}")
    print(f"카카오 API 사용 가능: {env_info['kakao_api_available']}")
